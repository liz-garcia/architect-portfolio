...
const projectTitleStyle =
  "duration-400 absolute z-10 transform place-self-center text-lg italic text-zinc-950 transition ease-in-out";
const linkStyle =
  "duration-400 z-40 size-full transform transition ease-in-out hover:opacity-10";
...
const FeaturedProjectsSection = ({ data }) => {
  return (
    <section id="featuredProjects" className={featuredProjectsStyle}>
      ...
      {/* ******** Every item represents a project. ******** */}
      {data.map((item, index) => (
        <div key={index} className={projectContainerStyle}>
          <Link to={`/portfolio/${item.projectId}`} className={linkStyle}>
            <Image
              src={item.refImg}
              altText={`Project ${index + 1}`}
              objectFit="cover"
              objectExtra=""
            />
          </Link>
          <p className={projectTitleStyle}>{item.title}</p>
        </div>
      ))}
    </section>
  );
